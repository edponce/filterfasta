...
int parseBlastTableIDs(hits_t *hits, char *line)
{
  ...
  char *tokenPtr;        // Save pointer for strtok()
  ...
  currQueryId = strtok_r(line, " \t", &tokenPtr);
  if( currQueryId == NULL )
  {
  ...
  currHitId = strtok_r(NULL, " \t", &tokenPtr);
  if( currHitId == NULL )
  {
...
int loadBlastTable(char *fn, hits_t *hits)
{
  ...
  int tEOF;              // Flag for end of table file
  ...
  double t_start;        // Start time for wall time measure
  double t_end;          // End time for wall time measure
  ...
  #pragma omp parallel shared(hits,tEOF,err) private(i) firstprivate(longest)
 {
  char *line;
  int lerr;
  ...
  for(i = 0LL; i < hits->nlines; i++)
  {
    ...
    if( tEOF == 0 && err == 0 )
    {
      ...
      if( line == NULL )
      {
        free(line);
        tEOF = 1;
        #pragma omp flush (tEOF)
      }

      // Parse BLAST query and hit IDs
      lerr = parseBlastTableIDs(hits, line);
      if( lerr != 0 )
      {
        free(line);
        err = lerr;
  ...
  t_end = get_wtime();
  fprintf(stdout, "OS walltime = %f seconds\n", t_end - t_start);
