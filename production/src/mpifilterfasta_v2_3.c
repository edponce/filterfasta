...
typedef struct st_args
{
  ...
  int            searchFlag;             // Flag for search file sequence extraction 
} args_t;
...
typedef struct st_iomap
{
  ...
  int           *fileOffs;    // File offsets for query file memory mappings
...
typedef struct st_hits
{
  ...
  int            pipeProg;     // Pipeline program after extracting sequences 
  int            searchFlag;   // Flag for search file sequnce extraction
...
int parseCmdline(int argc, char **argv, args_t *args, mpi_t *mpi)
{
  ...
  if( args->pipeProg != 0 && args->searchFlag != 0 )
  {
...
int extractQueries(args_t *args, iomap_t *iomap, query_t *query, hits_t *hits, mpi_t *mpi, long long int *bytesWritten, int *done)
{
  ...
  while( 1 )
  {
    ...
    if( mpi->procCnt == 1 )
    {
      ...
      if( hits->pipeProg != 0 || hits->searchFlag != 0 )
      {
    ...
    if( dataLen[0] > 0 )
    {
      ...
      while (curr_off < dataLen[0])
      {
        ...
        if( bytesRead != curr_sz )
          fprintf(stdout, "Master did not read chunk size correctly when combining outputs\n");
...
int partQueryFile(args_t *args, iomap_t *iomap, hits_t *hits, mpi_t *mpi)
{
  ...
  xcnt = 0LL;
  shift = 0LL;
  mpishift = (long long int)iomap->fileOffs[mpi->procRank*3+1];
...
int parseBlastTableIDs(hits_t *hits, char *line)
{
  ...
  { 
    ...
    if( strncmp(currQueryId, currHitId, HITS_ID_LEN) != 0 )
    {
...
int loadSearchIDs(char *fn, hits_t *hits)
{
  ...
  // Count number of lines in search file
  nch = 0LL;
  longest = 0LL;
  nlines = 0LL;
  while( 1 )
  ...
  longest++;     // plus 1 to account for null character added by fgets()
  hits->nlines = nlines;
  rewind(hits->tfd);
...
int setOffsQueryFile(iomap_t *iomap, int procCnt)
{
  ...
  partSz = (int)ceil((double)iomap->qfsz / procCnt);
  ...
  for(i = 0; i < procCnt; i++)
  {
    ...
    if( i < (procCnt - 1) ) 
    {
      ...
      if( i == 0 )
      {
        ...
        iomap->fileOffs[i*3] = 0;
        iomap->fileOffs[i*3+1] = 0; 
      }
      // Middle partitions
      else
      {
        // Compute a page size offset that is not after the end of the previous partition
        prevOffs = iomap->fileOffs[(i-1)*3] + iomap->fileOffs[(i-1)*3+1] + iomap->fileOffs[(i-1)*3+2];
        chunkOffs = (int)floor((double)prevOffs/seqChunks);

        // Set array of offsets 
        iomap->fileOffs[i*3] = seqChunks * chunkOffs;
        iomap->fileOffs[i*3+1] = prevOffs - iomap->fileOffs[i*3];
      }
      ...
      while( foundSeqBeg == 0 )
      {
        ...
        readOffs = iomap->fileOffs[i*3] + partSz - (seqChunks * j);
        if( readOffs < 0 )
        {
        ...
        for(c = seqChunks - 1; c >= 0; c--)
        {
          ...
          {
            // Set array of offsets
            iomap->fileOffs[i*3+2] = partSz - offset - iomap->fileOffs[i*3+1];
            if( iomap->fileOffs[i*3+2] == 0 )
            {
              fprintf(stdout, "Warning: too many processes for query file, adjusting...\n");
              free(buffer);
              return 2;
    ...
    else 
    {
      ...
      iomap->fileOffs[i*3] = seqChunks * chunkOffs;
      iomap->fileOffs[i*3+1] = prevOffs - iomap->fileOffs[i*3];
      iomap->fileOffs[i*3+2] = iomap->qfsz - (iomap->fileOffs[i*3] + iomap->fileOffs[i*3+1]);
    }
  }
...
int setOffs(iomap_t *iomap, mpi_t *mpi)
{
  ...
  iomap->fileOffs = (int *)malloc(sizeof(int) * mpi->procCnt * 3);
  ...
  if( mpi->procRank == 0 )
  {
    while( 1 )
    {
      ...
      memset(iomap->fileOffs, 0, sizeof(int) * procCnt * 3);
  ...
  err = MPI_Bcast(iomap->fileOffs, procCnt * 3, MPI_INT, 0, mpi->MPI_MY_WORLD);
  if( err != MPI_SUCCESS )
    return ERROR;
...
int main(int argc, char **argv)
{
  ...
  hits.searchFlag = args.searchFlag;
