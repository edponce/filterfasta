...
int computePartitionOffsets(long long int **offs, int *parts, int fd, long int sz, char sym)
{
  ...
  while( 1 )
  {
    ...
    *offs = realloc(*offs, (lparts * 3) * sizeof offs);
    ...
    maxparts = ceil((double)sz / chunks);
    multiplier = (long long int)floor(maxparts / lparts);
    //if( lparts > maxparts )
    if( multiplier < 1 )
    {
      lparts--;
...
fprintf(stdout, "Maxparts = %f, Multiplier = %ld, partSz = %lld\n", maxparts, multiplier, partSz);
      ...
      for(i = 0; i < lparts && err == 0; i++)
      {
        ...
        { 
          ...
          while( foundSymbol == 0 && err == 0 )
          {
            ...
            if( readOffs < 0 )
              fprintf(stdout, "Readoffset negative\n");
            ...
            if( bytesRead != chunks )
              fprintf(stdout, "Warning: data read does not match in partition offsets\n");
   
            char *psym = strrchr(buffer, (int)sym);
            char *pbuffer = &buffer[chunks-1];
            if( psym == NULL )
            {
              fprintf(stdout, "Symbol not found in chunk\n");
              poffset = poffset + chunks;
            }
            else
            {
              foundSymbol = 1;
              poffset = poffset + (long long int)(pbuffer - psym + 1);
              (*offs)[i*3+2] = partSz - poffset - (*offs)[i*3+1];
              if( (*offs)[i*3+2] < 1)
              {
                err = 2;
                lparts--;
                fprintf(stdout, "Warning: adjusted partitions based on data\n");
              }
            }
          } 
        }
