...
int getInputFilesComm(mpi_t *mpi, MPI_Comm *newComm)
{
  ...
  if( *newComm == MPI_COMM_NULL)
    fprintf(stdout, "Process %d not in new group\n", mpi->procRank);
  else
    fprintf(stdout, "Process %d in new group\n", mpi->procRank);
...
// Preprocess query file for memory map offsets
int setOffs(iomap_t *iomap, mpi_t *mpi, int pipeProg)
{
  ...
  // Only one process or pipeline mode, consider complete query file for all processes
  if( mpi->procCnt == 1 || pipeProg != 0 ) 
  {
...
int main(int argc, char **argv)
{
  ...
  // Initialize hit structure for pipeline mode
  ...
  err = setOffs(&iomap, &mpi, hits.pipeProg);
  if( err != 0 )
  {
