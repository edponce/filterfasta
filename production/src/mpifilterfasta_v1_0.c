...
int initQueryMap(long long int msz, long long int offset, iomap_t *iomap, mpi_t *mpi)
{
  ...
  // Debug:
  fprintf(stdout, "Process %d, msz = %lld, offset = %lld\n", mpi->procRank, msz, offset);
...
int partQueryFile(args_t *args, iomap_t *iomap, hits_t *hits, mpi_t *mpi)
{
  ...
  for(nmap = 0; nmap < nmaps && !done; nmap++)
  {
    ...
    if( nmap > 0LL )
    {
      ...
      // Debug:
      char cbuf[11];
      strncpy(cbuf, iomap->iMap, 10);
      cbuf[10] = '\0';
      fprintf(stdout, "Process %d, imap bytes = %s\n", mpi->procRank, cbuf);  
      strncpy(cbuf, iomap->fMap - 9, 10);
      cbuf[10] = '\0';
      fprintf(stdout, "Process %d, fmap bytes = %s\n", mpi->procRank, cbuf);  
...
if( mpi->procCnt > 1 && gatherData != 0 )
{
  ...
  {
    char *outbuf = NULL;
    long long int lbytesWrite;
    long long int lbytesRead;
    FILE *ofd;
    MPI_Status status;
    ...
    if( dataLen[0] > 0 )
    {
      outbuf = (char *)malloc(sizeof(char) * dataLen[0]);
      lbytesRead = fread(outbuf, sizeof(char), dataLen[0], iomap->ofd);
      if( lbytesRead != dataLen[0] )
        fprintf(stderr, "Error: bytes read do not match in fread(), partQueryFile()\n");
      lbytesWrite = fwrite(outbuf, sizeof(char), dataLen[0], ofd);
      if( lbytesWrite != dataLen[0] )
        fprintf(stderr, "Error: bytes written do not match in fwrite(), partQueryFile()\n");
    }

    for( i = 1; i < mpi->procCnt; i++)
    {
      if( dataLen[i] == 0 ) continue;
      if( dataLen[i] > dataLen[i-1] )
        outbuf = (char *)realloc(outbuf, sizeof(char) * dataLen[i]);
      MPI_Recv(outbuf, dataLen[i], MPI_CHAR, i, 0, mpi->MPI_MY_WORLD, &status);
      lbytesWrite = fwrite(outbuf, sizeof(char), dataLen[i], ofd);
      if( lbytesWrite != dataLen[i] )
        fprintf(stderr, "Error: bytes written do not match in fwrite(), partQueryFile()\n");
    }
  ...
  else
  {
    if( bytesWritten > 0 )
    {
      char *dataBuf;
      long long int lbytesRead;
      dataBuf = (char *)malloc(sizeof(char) * bytesWritten);
      rewind(iomap->ofd);
      lbytesRead = fread(dataBuf, sizeof(char), bytesWritten, iomap->ofd);
      if( lbytesRead != bytesWritten )
        fprintf(stderr, "Error: bytes read do not match in fread(), partQueryFile()\n");
      MPI_Send(dataBuf, bytesWritten, MPI_CHAR, 0, 0, mpi->MPI_MY_WORLD);
      free(dataBuf);
    }
  }
...
int setOffsQueryFile(iomap_t *iomap, mpi_t *mpi)
{
  ...
  if( buffer == NULL )
  {
    fprintf(stdout, "Error: failed to malloc() offset buffer\n");
    return ERROR;
  }
  ...
  fprintf(stdout, "File offsets: seqChunk = %d, partSz = %d\n", seqChunks, partSz);
  ...
  for(i = 0; i < mpi->procCnt; i++)
  {
    ...
    if( i == 0) 
    {
      ...
      while( foundSeqBeg == 0 )
      {
        ...
        if( fseek(iomap->qfd, readOffs, SEEK_SET) )
          fprintf(stderr, "\nError: failed fseek() in setOffsQueryFile\n");
      
        // Read a chunk from the file
        bytesRead = fread(buffer, sizeof(char), seqChunks, iomap->qfd);
        if( bytesRead != seqChunks )
          fprintf(stdout, "Warning: size of data read does not match, setOffsQueryFile()\n");
        ...
        for(c = seqChunks - 1; c >= 0; c--)
        {
          ...
          {
            // Set array of offsets
            mpi->fileOffs[i*3+2] = partSz - offset;
...
int adjustMPIProcs(mpi_t *mpi, int worldSz)
{
  ...
  MPI_Group_range_excl(worldGroup, 1, ranges, &myGroup);

  // Create a new communicator
  MPI_Comm_create(MPI_COMM_WORLD, myGroup, &mpi->MPI_MY_WORLD);
...
int main(int argc, char **argv)
{
  ...
  mpi_t mpi;
  mpi.MPI_MY_WORLD = MPI_COMM_WORLD;
  ...
  mpi.fileOffs = (int *)malloc(sizeof(int) * procCnt * 3);
  ...
  if( mpi.procCnt > 1 )
  {
    if( mpi.procRank == 0 )
    {
      while( 1 )
      {
        ...
        if( err == 2 )
        {
          ...
          mpi.procCnt = (int)ceil((double)mpi.procCnt / 2);
      ...
      // Debug: Print process offsets table
      int i;
      for(i = 0; i < mpi.procCnt; i++)
      {
        VERBOSE(fprintf(stdout, "Process %d, offset = [%d, %d, %d]\n", mpi.procRank, mpi.fileOffs[i*3], mpi.fileOffs[i*3+1], mpi.fileOffs[i*3+2]);)
      }
    }
    ...
    if( err != MPI_SUCCESS )
    {  
      fprintf(stderr, "Error: failed MPI broadcast of query file offsets\n\n");
      free(mpi.fileOffs);
      fclose(iomap.qfd);
      MPI_Finalize();
      return ERROR;
  ...
  // Free file offsets for memory mapping list
  free(mpi.fileOffs);
