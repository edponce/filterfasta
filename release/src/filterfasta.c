...
int parseCmdline(int argc, char **argv, args_t *args)
{
  ...
  while( 1 )
  {
    ...
    opt = getopt_long(argc, argv, ":q:o:c:l:a:b:t:p:v", longOpts, &optIdx);
...
int initQueryMap(long long int msz, long long int offset, iomap_t *iomap)
{
  ...
  // Lock memory map pages in memory
  iomap->iMap = (char *)mmap(NULL, msz, PROT_READ, MAP_PRIVATE|MAP_LOCKED, fileno(iomap->qfd), offset);
  if( iomap->iMap == MAP_FAILED )
  {
    // Debug statement
    fprintf(stderr, "\nWarning: failed locked memory map, attempting unlocked memory map\n");

    // Try creating unlocked memory map
    iomap->iMap = (char *)mmap(NULL, msz, PROT_READ, MAP_PRIVATE, fileno(iomap->qfd), offset);
    if( iomap->iMap == MAP_FAILED )
    {
      fprintf(stderr, "\n");
      perror("mmap()");
  ...
  madvise(iomap->iMap, msz, MADV_SEQUENTIAL);
...
{
  int err = 0;                 // Trap errors
  int seqSelect = 0;           // Number of sequences selected
  ...
  while( 1 )
  {
    ...
    if( hits->pipeProg != 0 )
    {
      ...
      for(i = 0LL; i < hits->htotal; i++)
      {
        ...
        // Compare hit ids, plus 1 to skip ">" at beginning of each sequence
