  ...
  while( 1 )
  {
    ...
    switch( opt )
    {
      ...
      case 'c':	// max sequence count
          ...
          {
            if( mpi->procRank == 0 )
              args->seqCnt = (long long int)ceil(testOpt/mpi->procCnt);
            else
              args->seqCnt = (long long int)floor(testOpt/mpi->procCnt);
          }
          break;
...
int partQueryFile(args_t *args, iomap_t *iomap, hits_t *hits, mpi_t *mpi)
{
  ...
  // Combine total of all sequences extracted
  if( mpi->procCnt > 1 )
  {
    MPI_Reduce(&iomap->xCnt, &allxCnt, 1, MPI_LONG_LONG_INT, MPI_SUM, 0, mpi->MPI_MY_WORLD);
    if( mpi->procRank == 0 )
      VERBOSE(fprintf(stdout, "Combined total sequences extracted = %lld\n", allxCnt);)
  }
      ...
      for(i = 0; i < lparts && err == 0; i++)
      {
        ...
        { 
          ...
          while( foundSymbol == 0 && err == 0 )
          {
            ...
            char *psym = strrchr(buffer, sym);
            char *pbuffer = &buffer[chunks-1];
            if( psym == NULL )
              tmpoff = tmpoff + chunks;
            else
              tmpoff = tmpoff + (long long int)(pbuffer - psym) + 1;
          } 
        }
        // Last partition
        else 
...
int setOffs(iomap_t *iomap, mpi_t *mpi)
{
  ...
  iomap->fileOffs = malloc((mpi->procCnt * 3) * sizeof iomap->fileOffs);
  ...
  if( mpi->procRank == 0 )
  {
    err = computePartitionOffsets(&iomap->fileOffs, &mpi->procCnt, fileno(iomap->qfd), iomap->qfsz, '>');
    if( err != 0 )
      mpi->procCnt = ERROR;
  }
